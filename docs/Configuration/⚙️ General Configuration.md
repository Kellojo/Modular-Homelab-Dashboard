...
config:
  ...
  disableSessionExpirationCheck: false
  background:
    url: ./background.jpg
    blur: 0.5rem
    brightness: 0.25
...
| `disableSessionExpirationCheck` | No       | Disable the automatic session expiration check (default: false, checks against status codes 401, 403, and 302).    |
